...
import com.surveysystem.util.enums.CPFValidationStatus;
...
public class PollVoteController {
	...
	private ResponseEntity<Response<PollVoteDto>> store(@RequestBody @Valid PollVote pollVote, BindingResult result) {
		...
		Optional<Poll> poll = pollRepository.findById(pollVote.getPoll().getId());
		...
		try {
			...
			Optional<Associado> associado = associadoRepository.findById(pollVote.getAssociado().getId());
